...
class inferProcessor(DataProcessor):
    def __init__(self, data_dir, shared_data_dir):
        ...
        fns = [fn for fn in fns if fn.endswith(".json") and "c3-" not in fn]


        for sid in range(3):
            data = []
            if sid == 1:
                with open(self.shared_data_dir + "/c3-d-train.json", "r", encoding="utf8") as f:
                    data += json.load(f)
                with open(self.shared_data_dir + "/c3-m-train.json", "r", encoding="utf8") as f:
                    data += json.load(f)
            if sid == 2:
                for fn in fns:
        ...
        if os.path.exists(cache_dir):
            ...
            for (i, d) in enumerate(data):
                ...
                for k in range(n_class):
                    if data[i][2 + k] == data[i][2 + n_class]:
                        answer = str(k)
