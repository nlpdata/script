...
from models.pytorch_modeling import BertConfig, BertForMultipleChoice, ALBertConfig, ALBertForMultipleChoice, BertForMultipleChoiceKL
...
class onProcessor(DataProcessor):
    def __init__(self, data_dir, shared_data_dir):
        self.D = [[], [], []]
        self.data_dir = data_dir
        self.shared_data_dir = shared_data_dir

        fns = os.listdir(self.data_dir)
        fn = [fn for fn in fns if fn.endswith(".json") and "c3-" not in fn][0]
        ...
        for sid in range(3):
            ...
            if sid != 0:
                for subtask in ["d", "m"]:
                    with open(self.shared_data_dir + "/c3-" + subtask + "-" + ["train.json", "dev.json", "test.json"][sid], "r",
                              encoding="utf8") as f:
                        data += json.load(f)
            if sid == 0:
                with open(self.data_dir + "/" + fn, "r", encoding="utf8") as f:
                    data += json.load(f)
            if sid == 0:
                random.shuffle(data)
            for i in range(len(data)):
                for j in range(len(data[i][1])):
                    ...
                    for k in range(len(data[i][1][j]["choice"]), n_class):
                        d += ['']
                    kl = []
                    for k in range(n_class):
                        if k < len(data[i][1][j]["choice"]) and data[i][1][j]["choice"][k] == data[i][1][j]["answer"]:
                            kl += [1.0]
                        else:
                            kl += [0]
                    if sid == 0:
                        for k in range(len(data[i][1][j]["kl"])):
                            kl[k] = data[i][1][j]["kl"][k]
                    d += [kl]
                    self.D[sid] += [d]
...
def main():
    ...
    parser.add_argument("--data_dir",
                        default='c3_release',
                        type=str)
    parser.add_argument("--shared_data_dir",
                        default='c3_release',
                        type=str)
    ...
    parser.add_argument("--bert_config_file",
                        default='roberta_wwm_ext_large/config.json',
                        type=str)
    parser.add_argument("--vocab_file",
                        default='roberta_wwm_ext_large/vocab.txt',
                        type=str)
    ...
    parser.add_argument("--init_checkpoint",
                        default='roberta_wwm_ext_large/pytorch_model.bin',
                        type=str,
    ...
    processor = processors[args.task_name.lower()](args.data_dir, args.shared_data_dir)
    ...
    if 'albert' in args.bert_config_file:
        ...
        model = BertForMultipleChoiceKL(bert_config, num_choices=n_class)
