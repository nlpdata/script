...
from models.pytorch_modeling_pretrain import BertConfig, BertForMultipleChoice, ALBertConfig, ALBertForMultipleChoice
...
class tbC3Processor(DataProcessor):
    def __init__(self, data_dir):
        ...
        for sid in range(3):
            ...
            # add behavior-based and scene-based mrc training data
            if sid == 0:
                with open(self.data_dir + "/behavior_no_xml_with_speakers_clean.json", "r", encoding="utf8") as f:
                    data += json.load(f)
                with open(self.data_dir + "/scene_no_xml_with_speakers_clean.json", "r", encoding="utf8") as f:
                    data += json.load(f)
...
class bC3Processor(DataProcessor):
    def __init__(self, data_dir):
        self.D = [[], [], []]
        self.data_dir = data_dir
        ...
        for sid in range(3):
            ...
            # add behavior-based mrc training data
            if sid == 0:
                # behavior within the speaker id
                #with open(self.data_dir + "/behavior_no_xml_with_speakers_clean.json", "r", encoding="utf8") as f:
                # behavior within the same turn, while in utterance
                #with open(self.data_dir + "/cat_gb.json", "r", encoding="utf8") as f:
                #    data += json.load(f)
                #with open(self.data_dir + "/cat_lb.json", "r", encoding="utf8") as f:
                #    data += json.load(f)
                with open(self.data_dir + "/cat_ib.json", "r", encoding="utf8") as f:
                    data += json.load(f)
                #with open(self.data_dir + "/cat_ct.json", "r", encoding="utf8") as f:
                #    data += json.load(f)
                #with open(self.data_dir + "/cat_ct_short2.json", "r", encoding="utf8") as f:
                #    data += json.load(f)
                #with open(self.data_dir + "/cat_mix_100k.json", "r", encoding="utf8") as f:
                #    data += json.load(f)
                #with open(self.data_dir + "/global_behavior_no_xml_with_speakers_clean.json", "r", encoding="utf8") as f:
                #    data += json.load(f)
                # use global and local behaviors together
                #if "2bc3" in self.data_dir or "2btc3" in self.data_dir or "2bmtc3" in self.data_dir:
                #    with open(self.data_dir + "/local_behavior_no_xml_with_speakers_clean.json", "r", encoding="utf8") as f:
                #        data += json.load(f)

                # add scene-based questions
                #if "2btc3" in self.data_dir:
                #    with open(self.data_dir + "/scene_no_xml_with_speakers_clean_pattern.json", "r", encoding="utf8") as f:
                #        data += json.load(f)
                # add mt questions
                #if "2bmtc3" in self.data_dir:
                #    filelist = ["nmtrace_split_all.json", "nmtdream_train.json", "nmtdream_test.json",
                #                "nmtdream_dev.json"]
                #    for _file in filelist:
                #        with open(self.data_dir + "/" + _file, "r", encoding="utf8") as f:
                #            data += json.load(f)
...
class transC3Processor(DataProcessor):
    def __init__(self, data_dir):
        self.D = [[], [], []]
        self.data_dir = data_dir
        ...
        for sid in range(3):
            ...
            # add scene-based mrc training data
            if sid == 0:
                with open(self.data_dir + "/scene_no_xml_with_speakers_clean.json", "r", encoding="utf8") as f:
                    data += json.load(f)
...
class bigC3Processor(DataProcessor):
    def __init__(self, data_dir):
        self.D = [[], [], []]
        self.data_dir = data_dir
        ...
        for sid in range(3):
            ...
            # add translated training set of race and dream
            if sid == 0:
                filelist = ["nmtrace_split_all.json", "nmtdream_train.json", "nmtdream_test.json", "nmtdream_dev.json"]
                for _file in filelist:
                    with open(self.data_dir + "/" + _file, "r", encoding="utf8") as f:
                        data += json.load(f)
...
class sceneProcessor(DataProcessor):
    def __init__(self, data_dir):
        random.seed(42)
        self.D = [[], [], []]
        #split1, split2 = 319700, 359662
        #split1, split2 = 212124, 238639   # 265155
        split1, split2 =  173019, 194646  # 216274
        self.data_dir = data_dir

        with open(self.data_dir + "/scene_no_xml_with_speakers_clean_pattern.json", "r", encoding='utf-8') as f:
            data = json.load(f)
            toshuffle = data[:split1]
            random.shuffle(toshuffle)
            data[:split1] = toshuffle
            for i in range(len(data)):
                if i < split1:
                    sid = 0
                elif i < split2:
    ...
    def _create_examples(self, data, set_type):
        """Creates examples for the training and dev sets."""
        cache_dir = os.path.join(self.data_dir, set_type + '_scene_examples.pkl')
        if os.path.exists(cache_dir):
            examples = pickle.load(open(cache_dir, 'rb'))
        else:
            examples = []
...
def main():
    ...
    parser.add_argument("--data_dir",
                        default='/data1/yudian/dataset/c3_release',
                        type=str)
    parser.add_argument("--task_name",
                        default='c3',
                        type=str)
    parser.add_argument("--bert_config_file",
                        default='/data1/yudian/pretrained/roberta_wwm_ext_large/config.json',
                        type=str)
    parser.add_argument("--vocab_file",
                        default='/data1/yudian/pretrained/roberta_wwm_ext_large/vocab.txt',
                        type=str)
    parser.add_argument("--output_dir",
                        default='./roberta_wwm_ext_large_c3/',
                        type=str)

    ## Other parameters
    parser.add_argument("--init_checkpoint",
                        default='/data1/yudian/pretrained/roberta_wwm_ext_large/pytorch_model.bin',
                        type=str,
    ...
    processors = {
        ...
        'c3': c3Processor,
        'bigc3': bigC3Processor,
        'tc3' : transC3Processor,
        'bc3' : bC3Processor,
        '2bc3': bC3Processor,
        '2btc3': bC3Processor,
        '2bmtc3': bC3Processor,
    ...
    angrycat = {}
    while True:
        for cuda_device in cuda_devices.split(","):
            if cuda_device in angrycat:
                continue
            try:
                total, used = check_mem(cuda_device)
                #print(cuda_device, total, used)
                total = int(total)
                #used = int(used)
                max_mem = int(total * 0.8)
                #block_mem = max_mem - used
                block_mem = max_mem - 0
                d = torch.device('cuda:' + cuda_device)
                angrycat[cuda_device] = torch.FloatTensor(256,1024,block_mem).cuda(d)
            except:
                #print("failed", cuda_device, "retrying ...")
                pass
        if len(angrycat) == len(cuda_devices.split(",")):
            print("done")
            break
    #'''
    #==============angrycat=========================
    ...
    if args.do_train:
        ...
        #==============angrycat=========================
        '''
        angrycat = [angrycat[x] for x in angrycat]
        while len(angrycat) != 0:
            del angrycat[0]
        del angrycat
        #'''
        #==============angrycat=========================
        ...
        for ie in range(int(args.num_train_epochs)):
            ...
            if args.do_eval:
                ...
                for input_ids, input_mask, segment_ids, label_ids in tqdm(eval_dataloader):
                    ...
                    with torch.no_grad():
                        tmp_eval_loss, logits = model(input_ids, segment_ids, input_mask, label_ids, True)
    ...
    try:
        ...
        logger.info("***** Cannot load model.pt *****")
    ...
    if args.do_eval:
        ...
        for input_ids, input_mask, segment_ids, label_ids in tqdm(test_dataloader):
            ...
            with torch.no_grad():
                tmp_test_loss, logits = model(input_ids, segment_ids, input_mask, label_ids, True)
