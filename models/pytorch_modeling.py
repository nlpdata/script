...
class BertForMultipleChoiceKL(PreTrainedBertModel):

    def __init__(self, config, num_choices=2):
        super(BertForMultipleChoiceKL, self).__init__(config)
        self.num_choices = num_choices
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(config.hidden_size, 1)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, return_logits=False):
        ...
        if labels is not None:
            ...
            loss_fct2 = KLDivLoss()
            logsoftmax = loss_fct1(reshaped_logits)
            loss = loss_fct2(logsoftmax, labels)
            if return_logits:
                return loss, reshaped_logits
            else:
                return loss
